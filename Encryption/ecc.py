...
mod = pow(2, 256) - pow(2, 32) - pow(2, 9) - pow(2, 8) - \
    pow(2, 7) - pow(2, 6) - pow(2, 4) - pow(2, 0)
...
plain_coordinates = EccCore.applyDoubleAndAddMethod(
    base_point[0], base_point[1], plaintext, a, b, mod)
...
publicKey = EccCore.applyDoubleAndAddMethod(
    base_point[0], base_point[1], secretKey, a, b, mod)
...
c1 = EccCore.applyDoubleAndAddMethod(
    base_point[0], base_point[1], randomKey, a, b, mod)

c2 = EccCore.applyDoubleAndAddMethod(
    publicKey[0], publicKey[1], randomKey, a, b, mod)
c2 = EccCore.pointAddition(
    c2[0], c2[1], plain_coordinates[0], plain_coordinates[1], a, b, mod)
...
new_point = EccCore.pointAddition(
    base_point[0], base_point[1], base_point[0], base_point[1], a, b, mod)  # 2P

# brute force method
for i in range(3, order):
    new_point = EccCore.pointAddition(
        new_point[0], new_point[1], base_point[0], base_point[1], a, b, mod)
